...
function Im2Seq() {
  const intro = [
    "Have you even been sitting in on a lecture, trying to understand what is going on, focussing very hard on the derivation the lecturer is speeding through in record time. Meanwhile, you're blindly scribbling down what looks like to be chicken scratches, but in reality is a elegant proof.",
    "You look back at your notes and now the process of cracking enigma begins. You say to youself, 'I wish my notes looked like those nice typed up math documents. At least then I could read my own notes.' And suddenly, an idea pops into your head.",
  ];
  const intro2 = [
  ...
  const data = [
    "Finding data for this project was a challenge in it of itself. There is no much handwritten math datasets that exist, at least none that I could find easily. Moreover, I needed these images to also have labels. I did not want to sift through hundreds of thousands of images and manually label with LaTeX.",
  ...
  const training2 = [
    "The exposure bias issue arrises because cross-entropy is token-level loss function. What if we had a different objective function? One that took into account the entire sequence, eliminating exposure bias?",
    "Enter BLEU score loss.",
    "BLEU score is a metric that quantifies 'how good' a single machine-translated segment is comapared to if a human were to do it. In traditional translation, these segments are typically sentence, but in our case we have chunks of LaTeX code. Scores are calculated for translated chunk by comparing them with a set of good quality reference translations. These scores are then averaged over the whole corpus to reach an estimate of the translation's overall quality. BLEU is inexpensive to compute and was one of the first metrics to show a high correlation with human judgements of quality.",
  ];
  const training3 = [
    "With this sequence-level loss, we can optimize the prediction of individual tokens within the context of the entire sequence. It also eliminates the exposure bias problem because the optimization is no longer based on each individual token, but on the entire sequence. All is well! Well, there are a couple of logistical issue. Using a sequence-level objective to train from a randomly initialized model state is computationally infeasible. Instead, we first train on a token-level objective, then use that as a starting point for sequence level training.",
